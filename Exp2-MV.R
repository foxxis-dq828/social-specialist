#define the majority vote model
...
integral_function<-function(c,i,n1,tau){
  w_list <- c(n1, n1, 1-n1, 0.5, n1, n1, 1-n1, 0.5)
  w<-w_list[i]
  alpha= 1+w*2*c
  beta= 1+(1-w)*2*c
...
# 7. Define the Objective Function for Optimization
objective_function <- function(par) {
  ...
  for (m in 1:nrow(all_choice)) {
    ...
    sf_lllist <- integral_function(c,condition,n1,tau)
