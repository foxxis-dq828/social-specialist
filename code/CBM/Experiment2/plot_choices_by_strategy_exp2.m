function plot_choices_by_strategy_exp2(data, cbm, bestModelPerSubj)
%PLOT_CHOICES_BY_STRATEGY_EXP2  Plot mean ratings by strategy group with predictions (Experiment 2).
%
%   PLOT_CHOICES_BY_STRATEGY_EXP2(DATA, CBM, BESTMODELPERSUBJ) groups
...
for gi = 1:numel(groupsToPlot)
    ...
    for k = 1:numel(grpIdx)
        ...
        for t = 1:T
            predictedMatGrp(k, t) = compute_predicted_mean_exp2(modelIdx, params, t);
        end
    end
...
title('Mean Ratings by Strategy with Predicted Means (Experiment 2)');
