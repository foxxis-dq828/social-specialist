%BMI_COMPARE_EXP2  Cluster participants into strategies and compare metrics across strategy clusters (Experiment 2).
%
%   This script loads subject-level metrics and the HBI responsibility
%   matrix for Experiment 2, assigns each subject to the model with maximum
...
confThr       = 0.6;
excludeUncert = true;
needVars      = {'av_pdiv','av_nodiv','t','median_t','div_t','nodiv_t'};

% -------- Load metrics table --------
T = readtable(data_file);
missingVars = setdiff(needVars, T.Properties.VariableNames);
assert(isempty(missingVars), 'Missing columns in CSV: %s', strjoin(missingVars, ', '));
...
% -------- Remove small-N groups --------
minN = 5;

[counts, gnames] = groupcounts(T.strategy);   % counts aligned with gnames
smallGroups = gnames(counts < minN);

if ~isempty(smallGroups)
    T = T(~ismember(T.strategy, smallGroups), :);
end
...
for k = 1:numel(needVars)
    ...
    sd = splitapply(@(x) std(x,'omitnan'),  y, Gid);  % SD
    Desc.(['mean_' m]) = mu;
    Desc.(['sd_'   m]) = sd;          
end
