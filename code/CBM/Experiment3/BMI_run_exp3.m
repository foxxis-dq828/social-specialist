%BMI_RUN_EXP3  Run hierarchical Bayesian model comparison for Experiment 3.
%
%   This script performs HBI model comparison across the candidate models
%   fitted to Experiment 3 data.  It assumes that the Laplace fits have
%   been produced by BMI_FIT_MODELS_EXP3.m.  The script executes:
%     1) Load the Experiment 3 data.
...
models    = {@mentalizing_model_exp3, @level0_model_exp3, @level1_model_exp3, @level2_model_exp3, @baseline_model_exp3};
fcbm_maps = {'lap_mentalizing_exp3.mat', 'lap_level0_exp3.mat', 'lap_level1_exp3.mat', 'lap_level2_exp3.mat', 'lap_baseline_exp3.mat'};
...
[mean_r2_best, r2_best] = get_mean_r_squared_bmi_exp3(1000, data, cbm, bestModelPerSubj);
fprintf('Best‑model‑per‑subject pooled mean r² = %.3f\n', mean_r2_best);

[~, winningModel] = max(pxp);
winningModelPerSubj = repmat(winningModel, numel(data), 1);
[mean_r2_win, r2_win] = get_mean_r_squared_bmi_exp3(1000, data, cbm, winningModelPerSubj);
fprintf('Winning‑model‑for‑all pooled mean r² = %.3f\n', mean_r2_win);

[~, bestModelPerSubj] = max(resp, [], 2);
[individual_mean_r2_best, individual_r2_indiv_boot_best] = get_individual_mean_r_squared_bmi_exp3(1000, data, cbm, bestModelPerSubj);
fprintf('Best‑model‑per‑subject (individual‑level) mean r² = %.3f\n', individual_mean_r2_best);

[~, winningModel] = max(pxp);
winningModelPerSubj = repmat(winningModel, numel(data), 1);
[individual_mean_r2_win, individual_r2_indiv_boot_win] = get_individual_mean_r_squared_bmi_exp3(1000, data, cbm, winningModelPerSubj);
fprintf('Winning‑model‑for‑all (individual‑level) mean r² = %.3f\n', individual_mean_r2_win);

% -------- Generate summary plots --------
[~, bestModelPerSubj] = max(resp, [], 2);
plot_choices_by_strategy_exp3(data, cbm, bestModelPerSubj);
plot_choice_distribution_beta_exp3(data, cbm, bestModelPerSubj);
...
plot_overall_means_exp3(data, cbm, bestModelPerSubj);
