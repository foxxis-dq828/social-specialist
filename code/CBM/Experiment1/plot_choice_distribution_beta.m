function plot_choice_distribution_beta(data, cbm, bestModelPerSubj)
%PLOT_CHOICE_DISTRIBUTION_BETA  Plot choice distributions and beta predictions.
%
%   PLOT_CHOICE_DISTRIBUTION_BETA(DATA, CBM, BESTMODELPERSUBJ) creates a
%   series of bar charts showing the distribution of actual choices (1…6)
%   made by participants for each trial and strategy, together with the
%   predicted distribution derived from the mean parameters of that
%   strategy.  Only strategies with at least six participants
%   (max responsibility ≥ 0.5) are included.  
...
sgtitle('Choice Distributions and Predicted Beta Distributions by Strategy and Trial');
end